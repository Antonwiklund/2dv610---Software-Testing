...
public class FunctionPointers {
	...
	public static void main(String[] args) {
		...
		Predicate<Integer> all = n -> (n instanceof Number) == true;
		...
		Predicate<Integer> aboveTen = n -> n > 10 == true; // Must be updated
	...
	public static void selectAndPrint(List<Integer> list, Predicate<Integer> predicate) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			int checkInt = list.get(i);
			if (predicate.test(checkInt) == true) {
	...
	private static List<Double> applyFunction(List<Double> numbers, Function<Double, Double> fx) {
		...
		for (int i = 0; i < numbers.size(); i++) {
			Double checkDouble = numbers.get(i);
			returnList.add(fx.apply(checkDouble));
		}
		return returnList;
	}
